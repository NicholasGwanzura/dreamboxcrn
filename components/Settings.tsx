...
export const Settings: React.FC = () => {
  ...
  const handleApproveUser = (role: 'Admin' | 'Manager' | 'Staff') => {
      if (approvalUser) {
          ...
          const subject = "Dreambox Account Approved";
          const body = `Hello ${approvalUser.firstName},\n\nYour account for Dreambox Advertising has been approved.\n\nCredentials\nUsername: ${approvalUser.username || approvalUser.email}\nRole: ${role}\n\nLogin here: ${window.location.origin}\n\nWelcome to the team,\nDreambox Admin`;
          
          const mailtoLink = `mailto:${approvalUser.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
          
          // Create temporary link to ensure consistent behavior across browsers
          const link = document.createElement('a');
          link.href = mailtoLink;
          link.click();
          
          setApprovalUser(null);
          alert(`User approved! Your email client has been opened to notify ${approvalUser.email}.`);
      }
  };
                ...
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-white/20">
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            <div>
                                <p className="text-xs font-bold uppercase text-slate-400 mb-3 text-center">Select Role & Send Email</p>
                                <div className="grid grid-cols-1 gap-3">
                                    <button onClick={() => handleApproveUser('Staff')} className="py-3 px-4 bg-white border border-slate-200 hover:border-indigo-500 hover:text-indigo-600 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-2"><Mail size={16}/> Approve as Staff</button>
                                    <button onClick={() => handleApproveUser('Manager')} className="py-3 px-4 bg-white border border-slate-200 hover:border-indigo-500 hover:text-indigo-600 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-2"><Mail size={16}/> Approve as Manager</button>
                                    <button onClick={() => handleApproveUser('Admin')} className="py-3 px-4 bg-white border border-slate-200 hover:border-indigo-500 hover:text-indigo-600 rounded-xl text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-2"><Mail size={16}/> Approve as Administrator</button>
                                </div>
                                <p className="text-[10px] text-slate-400 text-center mt-3">This will launch your email client to send login details.</p>
                            </div>
