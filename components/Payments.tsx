...
import { getClients, getInvoices, getClientFinancials, getTransactions, getContracts, getBillboards, addInvoice, markInvoiceAsPaid, getUpcomingBillings, deleteInvoice, updateInvoice, pullAllDataFromSupabase } from '../services/mockData';
import { generateStatementPDF, generatePaymentSchedulePDF } from '../services/pdfGenerator';
import { Client, Invoice } from '../types';
import { DollarSign, FileText, Download, CheckCircle, AlertCircle, Search, CreditCard, X, Check, Filter, Hash, Wallet, Building, Clock, Calendar, Trash2, RefreshCw, Edit2, Save } from 'lucide-react';
...
export const Payments: React.FC = () => {
    ...
    useEffect(() => { const loadData = () => { const allDocs = getInvoices(); setAllInvoices(allDocs.filter(i => i.type === 'Invoice')); setAllReceipts(allDocs.filter(i => i.type === 'Receipt')); }; loadData(); }, [activeTab, selectedInvoice, editingInvoice]);
    ...
    const handleUpdateInvoice = (e: React.FormEvent) => {
        ...
        if(editingInvoice) {
            ...
            setAllInvoices(allDocs.filter(i => i.type === 'Invoice'));
        }
    };
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              {editingInvoice && (
                <div className="fixed inset-0 z-[200] overflow-y-auto">
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md transition-opacity" onClick={() => setEditingInvoice(null)} />
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                        <div className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-white/20">
                            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
                                <h3 className="text-xl font-bold text-slate-900">Edit Invoice</h3>
                                <button onClick={() => setEditingInvoice(null)} className="text-slate-400 hover:text-slate-600 p-2 hover:bg-slate-100 rounded-full transition-colors"><X size={20} /></button>
                            </div>
                            <form onSubmit={handleUpdateInvoice} className="p-8 space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="group relative">
                                        ...
                                        <input type="date" value={editingInvoice.date} onChange={(e) => setEditingInvoice({...editingInvoice, date: e.target.value})} className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 focus:border-slate-800 focus:ring-2 focus:ring-slate-100 outline-none transition-all font-medium" required />
                                    </div>
                                    <div className="group relative">
                                        <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Status</label>
                                        <select value={editingInvoice.status} onChange={(e) => setEditingInvoice({...editingInvoice, status: e.target.value as any})} className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 focus:border-slate-800 focus:ring-2 focus:ring-slate-100 outline-none transition-all font-medium">
                                ...
                                <div className="group relative">
                                    ...
                                    <select value={editingInvoice.type} onChange={(e) => setEditingInvoice({...editingInvoice, type: e.target.value as any})} className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 focus:border-slate-800 focus:ring-2 focus:ring-slate-100 outline-none transition-all font-medium">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="group relative">
                                        <label className="block text-xs font-bold uppercase text-slate-400 mb-2">Subtotal ($)</label>
                                        <input type="number" min="0" step="0.01" value={editingInvoice.subtotal} onChange={(e) => setEditingInvoice({...editingInvoice, subtotal: parseFloat(e.target.value) || 0, total: (parseFloat(e.target.value) || 0) + editingInvoice.vatAmount})} className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 focus:border-slate-800 focus:ring-2 focus:ring-slate-100 outline-none transition-all font-medium" required />
                                    </div>
                                    <div className="group relative">
                                        <label className="block text-xs font-bold uppercase text-slate-400 mb-2">VAT Amount ($)</label>
                                        <input type="number" min="0" step="0.01" value={editingInvoice.vatAmount} onChange={(e) => setEditingInvoice({...editingInvoice, vatAmount: parseFloat(e.target.value) || 0, total: editingInvoice.subtotal + (parseFloat(e.target.value) || 0)})} className="w-full px-4 py-3 border border-slate-200 rounded-xl text-slate-800 focus:border-slate-800 focus:ring-2 focus:ring-slate-100 outline-none transition-all font-medium" required />
                                    </div>
                                </div>
                                <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                                    <div className="flex justify-between items-center">
                                        <span className="text-sm font-bold text-indigo-700">Total Amount</span>
                                        <span className="text-2xl font-bold text-indigo-900">${editingInvoice.total.toLocaleString()}</span>
                                    </div>
                                </div>
                                <button type="submit" className="w-full px-4 py-4 text-white bg-slate-900 rounded-xl hover:bg-slate-800 flex items-center justify-center gap-2 shadow-xl font-bold uppercase tracking-wider transition-all hover:scale-105"><Save size={18} /> Update Invoice</button>
                            </form>
                        </div>
                    </div>
                </div>
